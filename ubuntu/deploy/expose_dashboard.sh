...
function expose_dashboard() {
    ...
    PROM_PORT_HTTP=$(kubectl --namespace ${MONITORING_NAMESPACE} get service kube-prom-stack-kube-prome-prometheus -o 'jsonpath={.spec.ports[?(@.name=="web")].port}')
    PATCH='{"data": {"'${PROM_EXT_PORT_HTTP}'": "'${MONITORING_NAMESPACE}'/kube-prom-stack-kube-prome-prometheus:'${PROM_PORT_HTTP}'"}}'
    ...
    GRAF_PORT_HTTP=$(kubectl --namespace ${MONITORING_NAMESPACE} get service kube-prom-stack-grafana -o 'jsonpath={.spec.ports[?(@.name=="http")].port}')
    PATCH='{"data": {"'${GRAF_EXT_PORT_HTTP}'": "'${MONITORING_NAMESPACE}'/kube-prom-stack-grafana:'${GRAF_PORT_HTTP}'"}}'
    kubectl patch configmap nginx-ingress-tcp-microk8s-conf --namespace ingress --patch "${PATCH}"
