...
function loadmultidomaintestibnfs()
    ...
    controllerips = ["127.0.0.1" , "10.41.83.198", "10.41.83.77"]
    ...
    ibnfs = [
        let
            ...
            context = get_contexts(controllerurls[i])["contexts"]
            if isempty(context)
                setup_context_topology(sdncontroller)
                create_graph_with_devices(ibnag, sdncontroller)
            end

            if !isfile(devicemapfiles[i])
                save_device_map(devicemapfiles[i], sdncontroller)
            end

            load_device_map!(devicemapfiles[i], sdncontroller)
