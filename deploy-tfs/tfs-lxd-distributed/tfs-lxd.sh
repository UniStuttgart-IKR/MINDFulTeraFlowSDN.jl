...
if [[ "$PART1_COMPLETE" == "true" && "$DEPLOYMENT_COMPLETE" == "false" ]]; then
  echo "[lxd] Copying local MINDFulTeraFlowSDN.jl to VM..."
  ${USE_SUDO} lxc file push -r "../../../MINDFulTeraFlowSDN.jl" "${VM_NAME}/home/tfsuser/"
  ${USE_SUDO} lxc exec "${VM_NAME}" -- bash -c 'chown -R tfsuser:tfsuser /home/tfsuser/MINDFulTeraFlowSDN.jl'
